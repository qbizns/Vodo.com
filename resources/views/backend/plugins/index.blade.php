{{-- Installed Plugins List (Screen 2) --}}
{{-- PJAX Layout for SPA navigation --}}

@extends('backend.layouts.pjax')

@section('title', __t('plugins.installed'))
@section('page-id', 'system/plugins')
@section('require-css', 'plugins')

@section('header', __t('plugins.installed'))
...
<div class="plugins-page">
    ...
    <div class="plugins-toolbar">
        ...
        <div class="plugins-count">
            {{ __t('plugins.showing_of', ['showing' => $plugins->count(), 'total' => $stats['total']]) }}
        </div>
    </div>
                        ...
                        <tr class="plugin-row" data-plugin-id="{{ $plugin->id }}" data-plugin-slug="{{ $plugin->slug }}">
                            ...
                            <td class="text-right">
                                <div class="actions-dropdown">
                                    <button type="button" class="action-menu-btn" data-plugin="{{ $plugin->slug }}">
    ...
    <div class="plugins-stats-bar">
        ...
        <div class="stat-divider"></div>
        <div class="stat-item">
            <span class="stat-value {{ $stats['updates'] > 0 ? 'stat-warning' : '' }}">{{ $stats['updates'] }}</span>
            <span class="stat-label">{{ __t('plugins.updates_available') }}</span>
        </div>
        <div class="stat-divider"></div>
        <div class="stat-item">
            <span class="stat-value {{ $stats['licenses_expiring'] > 0 ? 'stat-danger' : '' }}">{{ $stats['licenses_expiring'] }}</span>
