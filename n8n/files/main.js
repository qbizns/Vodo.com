...
function renderConnections() {
    ...
    state.connections.forEach(conn => {
        ...
        const path = createBezierPath(
            fromNode.x + 150, fromNode.y + 30,
            toNode.x, toNode.y + 30
        );
        
        $group.append(`<path class="connection-path" d="${path}" data-from="${conn.from}" data-to="${conn.to}"/>`);
    });
}

function createBezierPath(x1, y1, x2, y2) {
    const dx = Math.abs(x2 - x1);
    const cp = Math.min(Math.max(dx * 0.5, 50), 150);
...
function handleOutputHandleClick(e) {
    ...
    // Open action panel for adding connected node
...
function handleMouseMove(e) {
    if (state.isDraggingCanvas) {
        ...
        if (fromNode) {
            const pos = screenToCanvas(e.clientX, e.clientY);
            updateTempConnection(fromNode.x + 150, fromNode.y + 30, pos.x, pos.y);
        }
    }
}
